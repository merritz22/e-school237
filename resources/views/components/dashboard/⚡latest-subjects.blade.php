...
new class extends Component
{
    ...
    {
        $this->latest_subjects = EvaluationSubject::with('subject') // catégorie ou matière
            ->latest()
            ->take(8)
            ->get();
    }
...
<div class="mb-12">
    <div class="flex justify-between items-center mb-8">
        <flux:heading size="2xl" class="font-bold text-gray-900">
            Derniers Sujets d'évaluation
        </flux:heading>
        <flux:button href="{{ route('subjects.index') }}" icon:trailing="arrow-right">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        @empty
            <div class="col-span-4 text-center py-12">
                <flux:text class="text-gray-500">Aucun sujet disponible.</flux:text>
            </div>
        @endforelse
    </div>
</div>